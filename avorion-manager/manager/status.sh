...
#First Try
while [ $time -lt 20 ]; do
  PlayerMemory=$(awk "/${D}/,/min. update/" ${SCRIPTPATH}/server.log | grep 'players in memory')
...
if [ "${success}" = false ]; then
  ...
  time=0
  while [ $time -lt 20 ]; do
    SaveStatus=$(cat ${SCRIPTPATH}/server.log | awk "/${D}/,/Triggered saving of all server data/" | grep 'Triggered saving of all server data')
    if [ "${SaveStatus}" ]; then
      success=true
      break;
    fi
...
PlayerMemory=$(awk "/${D}/,/min. update/" ${SCRIPTPATH}/server.log | grep 'players in memory' | sed -e 's/.*| //g')
FactionsMemory=$(awk "/${D}/,/min. update/" ${SCRIPTPATH}/server.log | grep 'factions in memory' | sed -e 's/.*| //g')
SectorsMemory=$(awk "/${D}/,/min. update/" ${SCRIPTPATH}/server.log | grep 'sectors in memory' | sed -e 's/.*| //g')
ScriptsMemory=$(awk "/${D}/,/min. update/" ${SCRIPTPATH}/server.log | grep 'Memory used by scripts' | sed -e 's/.*| //g')
ServerLoad=$(awk "/${D}/,/min. update/" ${SCRIPTPATH}/server.log | grep 'server load' | sed -e 's/ 0%/ 0.01%/g' -e 's/.*| //g')
AvgUpdate=$(awk "/${D}/,/min. update/" ${SCRIPTPATH}/server.log | grep 'avg. update' | sed -e 's/.*| //g')
MaxUpdate=$(awk "/${D}/,/min. update/" ${SCRIPTPATH}/server.log | grep 'max. update' | sed -e 's/.*| //g')
MinUpdate=$(awk "/${D}/,/min. update/" ${SCRIPTPATH}/server.log | grep 'min. update' | sed -e 's/.*| //g')
PlayersCount=$(awk "/${D}/,/online players/" ${SCRIPTPATH}/server.log | grep 'online players (' | sed -e 's/online players (//' -e 's/).*//' -e 's/.*| //g' | tr -d '[:blank:]')
PlayersNames=$(awk "/${D}/,/online players/" ${SCRIPTPATH}/server.log | grep 'online players ('| sed -e 's/.*://' | tr -d '[:blank:]')
